...
xt=(0:1:n);
for h = 1:n+1
     xt(h)=cos((2*(h-1)+1)*pi/(2*n+2));
end 
x = [I(1):(I(2)-I(1))/n:I(2)];

InterpolationAD(f1,I,5,x)
InterpolationTscheby(f1,I,5,xt)
InterpolationAD(f2,I,5,x)
InterpolationTscheby(f2,I,5,xt)
end
...
function [] = InterpolationAD(f,I,n,x)
  x = [I(1):(I(2)-I(1))/n:I(2)];
  y = f(x);
  f_interpol = (1:n+1);
...
%Aufgabe 3c)
function [] = InterpolationTscheby(f,I,n,xt)
  xt=(0:1:n);
  for h = 1:n+1
     xt(h)=cos((2*(h-1)+1)*pi/(2*n+2));
end
  ...
  for l = 1:n+1
    f_interpol(l) = AitkenNeville(xt,y,xt(l));
  end
