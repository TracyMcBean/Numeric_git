...
function [Integral] = run_8_2()
    ...
    %Anwendung der Funktion gauss_quadratur zu 
    %f(x) = 2*log(x/2+1)/(x^2+4) in [0,2]
    %Dabei wird die Quadratur fuer verschiedene Werte n =1,2,3,4 und
    % m = 1,2,...,10 durchgefuehrt.
    %Anschliessend wird der Fehler logarithmisch ueber m Aufgetragen 
    %und fuer alle n geplottet.
    %Daraus laesst sich die Konvergenzordnung ablesen.(Aufgabe 2b)
...
%Implementierung der Gaussquadratur mit Koeffizienten aus Bronstein:(Aufgabe 2a)
function [y] = gauss_quad(f,a,b,n,m)
    x = linspace(a,b,m+1);
    xi= zeros(4);
    ai = zeros(4);

    % Gewichte und Stuetzstellen
    xi(1,1)=0;
